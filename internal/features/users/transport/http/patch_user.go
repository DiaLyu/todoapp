	...
	if r.PhoneNumber.Set {
		if r.PhoneNumber.Value != nil {
			...
			if !strings.HasPrefix(*r.PhoneNumber.Value, "+") {
				return fmt.Errorf("`PhoneNumber` must start to `+` symbol")
			}
		}
	}
...
// @Description   Изменение данных пользователя по ID пользователя
// @Description   ### Логика обновления полей (Three-state logic:)
// @Description   1. **Поле не передано**: `phone_number` игнорируется, значение в БД не изменяется
// @Description   2. **Явно передано значение**: `"phone_number": "+73334445566"` - устанавливает новое значение
...
// @Success       200 {object} PatchUserResponse "Данные пользователя успешно изменены"
