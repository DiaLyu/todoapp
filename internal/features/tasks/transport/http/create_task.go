...
type CreateTasksRequest struct {
	Title        string  `json:"title" validate:"required,min=1,max=100"         example:"Провести уборку дома"`
	Description  *string `json:"description" validate:"omitempty,min=1,max=1000" example:"Сделать до обеда"`
...
// @Accept        json
// @Produce       json
// @Param         request body CreateTasksRequest true "CreateTask тело запроса"
...
func (h *TasksHTTPHandler) CreateTask(rw http.ResponseWriter, r *http.Request) {
	...
	if err := core_http_request.DecodeAndValidateRequest(r, &request); err != nil {
		responseHandler.ErrorResponse(
			err,
			"failed to decode and validate HTPP request",
		)
